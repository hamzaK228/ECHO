        ...
        <div className="max-w-5xl mx-auto px-6 py-8 space-y-8">
          <div className="space-y-2">
            ...
            <p className="text-on-surface-variant font-body">
              {queue.length > 0
                ? `${queue.length} человек ждут поддержки`
                : 'Сейчас никто не ждёт. Ты можешь отдохнуть 🌿'}
            </p>
          </div>
          ...
          {queue.length === 0 ? (
            <div className="text-center py-16 space-y-4">
              <div className="w-20 h-20 rounded-full bg-surface-container-low flex items-center justify-center mx-auto">
                <span className="material-symbols-outlined msf text-on-surface-variant text-4xl">spa</span>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {queue.map((item) => (
                <div key={item.roomId} className="bg-surface-container-lowest rounded-xl p-6 shadow-sm border border-outline-variant/10 hover:shadow-md transition-shadow space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-full font-headline font-bold text-sm bg-gradient-to-br from-secondary to-secondary-container text-on-secondary-container shadow-md shadow-secondary/20 hover:scale-[1.02] active:scale-95 transition-all"
                  >
                    Принять чат →
                  </button>
                </div>
